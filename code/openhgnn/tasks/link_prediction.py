...
class LinkPrediction(BaseTask):
    ...
    def __init__(self, args):
        ...
        # self.train_hg, self.val_hg, self.test_hg, self.neg_val_graph, self.neg_test_graph, self.train_triplets, self.val_triplets, self.test_triplets = self.dataset.get_split()
        # self.val_triplets = th.tensor(random.sample(self.val_triplets.tolist(), math.ceil(self.val_triplets.shape[0]*0.01)))
        # self.test_triplets = th.tensor(random.sample(self.test_triplets.tolist(), math.ceil(self.test_triplets.shape[0]*0.1)))
    ...
    def evaluate(self, n_embedding, r_embedding=None, mode='test'):
        ...
        if self.evaluation_metric == 'acc':
            ...
            # print(self.dataset.train_triplets, self.dataset.valid_triplets, self.dataset.test_triplets)
            return mrr_matrix
            ...
            # print(n_embedding)
            # print(r_embedding)
            # self.ScorePredictor = HeteroTransXPredictor(1)
            # mrr_matrix = self.evaluator.mrr_(n_embedding, r_embedding,
            #                                  self.train_triplets, self.val_triplets,
            #                                  self.test_triplets,
            #                                  score_predictor=self.ScorePredictor, hits=[10, 100],
            #                                  filtered=getattr(self, 'filtered', 'raw'), eval_mode=mode)
            ...
            pr_auc = self.evaluator.cal_pr_auc(th.cat((p_label, n_label)).cpu(), th.cat((p_score, n_score)).cpu())
            acc = self.evaluator.cal_acc(th.cat((p_label, n_label)).cpu(), th.cat((p_score, n_score)).cpu())
            f1 = self.evaluator.f1_score(th.cat((p_label, n_label)).cpu(), th.cat((p_score, n_score)).cpu())
            loss = F.binary_cross_entropy_with_logits(th.cat((p_score, n_score)), th.cat((p_label, n_label)))
            # dict_a = dict(roc_auc=roc_auc, loss=loss, pr_auc=pr_auc, acc=acc, f1=f1)
            # dict_b = mrr_matrix
            # dict_a.update(dict_b)
            # return dict_a
            return dict(roc_auc=roc_auc, loss=loss, pr_auc=pr_auc, acc=acc, f1=f1)
        else:
