...
class Evaluator():
    ...
    def f1_score(self, y_label, y_pred):
        y_pred = np.around(y_pred,0).type(th.int) # just for link_prediction need
    ...
    def cal_pr_auc(self, y_true, y_pred):
...
def perturb_and_get_rank(n_embedding, r_embedding, eval_triplets, triplets_to_filter, score_predictor, filtered, preturb_side):
    ...
    num_entities = n_embedding['protein'].shape[0]
    ...
    for idx in eval_range:
        ...
        emb_s = n_embedding['protein'].cpu().index_select(0, select_s)
        # emb_r = r_embedding[int(target_r)]
        emb_r = r_embedding['interacts'].cpu()
        # emb_o = n_embedding[select_o]
        emb_o = n_embedding['protein'].cpu().index_select(0, select_o)
        # print(emb_s, emb_r, emb_o)
