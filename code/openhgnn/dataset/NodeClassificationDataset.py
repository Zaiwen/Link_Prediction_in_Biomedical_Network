...
class NodeClassificationDataset(BaseDataset):
    ...
    # def get_split(self, validation=False):
        r"""
...
class HIN_NodeClassification(NodeClassificationDataset):
    ...
    def load_HIN(self, name_dataset):
        if name_dataset == 'PPI_graph':
            data_path = './openhgnn/dataset/PPI_graph.bin'
            category = 'protein'
            num_classes = 50
            ...
            self.meta_paths_dict = {
                'PP': [('protein', 'interacts', 'protein')]
            }
            # self.in_dim = g.ndata['h'][category].shape[1]
        elif name_dataset == 'demo_graph':
