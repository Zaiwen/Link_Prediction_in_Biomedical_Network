...
class HIN_LinkPrediction(LinkPredictionDataset):
    ...
    def load_HIN(self, dataset_name):
        ...
        if dataset_name == 'NDFRT_DDA':
            g, _ = dgl.load_graphs('./openhgnn/dataset/NDFRT_DDA.bin')
            g = g[0]
            g = add_reverse_edges(g)
            self.target_link = [('drug', 'associates', 'disease')]
            self.target_link_r = [('disease', 'associates-rev', 'drug')]
            self.node_type = ['drug', 'disease']
            self.meta_paths_dict = {
                'D1D2D1':[('drug', 'associates', 'disease'), ('disease', 'associates-rev', 'drug')],
                'D2D1D2':[('disease', 'associates-rev', 'drug'), ('drug', 'associates', 'disease')]
            }
    ...
    #             test_index = random_int[int(num_edges * val_ratio):int(num_edges * (test_ratio + val_ratio))]
    #             test_edge = self.g.find_edges(test_index, etype)
    #             train_index = random_int[int(num_edges * (test_ratio + val_ratio)):]
    #             train_edge = self.g.find_edges(train_index, etype)
    ...
    #             train_triplets = th.stack((th.unsqueeze(train_edge[0], dim=1), th.unsqueeze(th.zeros(train_edge[0].shape, dtype=th.int32), dim=1), th.unsqueeze(train_edge[1], dim=1)), 2).squeeze(1)
    #             val_triplets = th.stack((th.unsqueeze(val_edge[0], dim=1), th.unsqueeze(th.zeros(val_edge[0].shape, dtype=th.int32), dim=1), th.unsqueeze(val_edge[1], dim=1)), 2).squeeze(1)
    #             test_triplets = th.stack((th.unsqueeze(test_edge[0], dim=1), th.unsqueeze(th.zeros(test_edge[0].shape, dtype=th.int32), dim=1), th.unsqueeze(test_edge[1], dim=1)), 2).squeeze(1)
    ...
    #     # todo: val/test negative graphs should be created before training rather than
    #     #  create them dynamically in every evaluation.
    #     return train_graph, val_graph, test_graph, None, None, train_triplets, val_triplets, test_triplets
